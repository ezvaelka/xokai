...
import { inviteTeacher }            from '@/app/actions/teachers'
...
export default function InviteTeacherForm({ onClose, onSuccess }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-xk-border">
          <h2 className="font-heading text-lg font-bold text-xk-text">Invitar maestro/a</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.first_name} onChange={e => set('first_name', e.target.value)} placeholder="Ana" className={inputClass} />
            </div>
            <div>
              <label className="block text-xs font-medium text-xk-text-muted uppercase tracking-wider mb-1.5">Apellido *</label>
              <input value={form.last_name} onChange={e => set('last_name', e.target.value)} placeholder="García" className={inputClass} />
            </div>
          </div>
          ...
          <p className="text-xs text-xk-text-muted">
            Se enviará un correo de invitación. El maestro/a creará su contraseña al aceptar.
          </p>
