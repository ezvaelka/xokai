...
import { updateSchoolPlan, type SchoolPlan } from '@/app/actions/sysadmin'
...
export default function SchoolPlanPanel({ schoolId, currentPlan, trialEndsAt }: Props) {
  ...
  return (
    <div className="xk-surface-elevated p-5">
      <h3 className="text-sm font-semibold text-xk-text mb-1">Plan contratado</h3>
      {daysLeft !== null && (
        <p className="text-xs text-xk-text-muted mb-4">
          Trial: <span className="xk-num font-semibold text-xk-text">{daysLeft}</span> días restantes
      ...
      {plan === 'suspended' && (
        <p className="text-xs text-orange-700 bg-orange-50 border border-orange-200 rounded-lg px-3 py-2 mt-3">
          Al suspender, la escuela perderá acceso de inmediato.
        </p>
      )}
    </div>
  )
