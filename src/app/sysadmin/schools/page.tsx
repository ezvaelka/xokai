  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-xk-card border border-xk-border rounded-2xl overflow-hidden">
        {schools.length === 0 ? (
          <div className="p-10 text-center text-sm text-xk-text-muted">
              ...
              <tbody>
                {schools.map((s) => {
                  ...
                  return (
                    <tr
                      key={s.id}
                      className="border-b border-xk-border last:border-0 hover:bg-xk-subtle/50 transition-colors"
                    >
                      <td className="px-4 py-3">
                        <Link
                          href={`/sysadmin/schools/${s.id}`}
                          className="font-medium text-xk-text hover:text-xk-accent"
                        >
                          {s.name}
                        </Link>
                        {s.email && (
                          <p className="text-xs text-xk-text-muted mt-0.5">{s.email}</p>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {s.director_name ? (
                          <>
                            <p className="text-xk-text">{s.director_name}</p>
                            {s.director_email && (
